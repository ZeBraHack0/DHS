...
int main()
{
	...
	for(int i = 0; i<12; i++)
	{
		...
		for (int datafileCnt = START_FILE_NO; datafileCnt <= END_FILE_NO; ++datafileCnt)
		{
			...
			//trace<<"file:"<<datafileCnt<<" "<<packet_cnt<<endl;
			...
			for (int t = 0; t < test_cycles; ++t)
			{
				...
				for (int i = 0; i < packet_cnt; ++i)
				{
					...
					//trace<<i/window<<" "<<hg[trace_hash % BUCKET_NUM].query(finger_print(trace_hash), trace_hash)<<endl;
					...
					if(i && i % window == 0)
					{
						...
						if(1)
						{
							...
							for(auto it:gb_cnt)
							{
								...
								if(f1 != f2)trace<<"error:"<<it.first<<" "<<it.second<<" "<<efreq<<endl;
							}
				...
				for(auto it:gb_cnt)
				{
					if(it.second > 255)l3 += it.second;
					topk.push_back(make_pair(it.first, it.second));
				}
				cout<<"l3 proportion: "<<l3/packet_cnt<<endl;
				...
				for(int i =0; i<k; i++)
				{
					...
					if(topk[i].second > 255 && (efq < topk[i].second/2 || efq/2 > topk[i].second))
					{
						...
						state<<"severe error: "<<topk[i].second<<" "<<topk[i].second<<" "<<efq<<endl;
